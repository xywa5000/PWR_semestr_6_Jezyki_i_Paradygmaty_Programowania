...
void test_factorial(int input, long long answer) {
    ...
    long long result_recursive = factorial_recursive(input);
...
void test_gcd(unsigned int input_x, unsigned int input_y, unsigned int answer) {
    unsigned int result_iterative = gcd_iterative(input_x, input_y);
    unsigned int result_recursive = gcd_recursive(input_x, input_y);
...
void test_diophantine(int input_a, int input_b, int input_c) {
    ...
    bool iterative_condition = (sol_iterative.err == true && input_c % gcd_iterative(input_a, input_b) != 0) || ((input_a * sol_iterative.x) + (input_b * sol_iterative.y) == input_c);
    bool recursive_condition = (sol_recursive.err == true && input_c % gcd_recursive(input_a, input_b) != 0) || ((input_a * sol_recursive.x) + (input_b * sol_recursive.y) == input_c);
