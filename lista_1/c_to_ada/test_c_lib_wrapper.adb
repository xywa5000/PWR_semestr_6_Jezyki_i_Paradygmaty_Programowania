...
with C_Lib_Wrapper; use C_Lib_Wrapper;

procedure Test_C_Lib_Wrapper is
   ...
   begin
      Sol_Iterative := Diophantine_Iterative_C(Input_A, Input_B, Input_C);
      Sol_Recursive := Diophantine_Recursive_C(Input_A, Input_B, Input_C);

      Iterative_Condition :=
         (Sol_Iterative.Err and then Input_C mod GCD_Iterative_C(Input_A, Input_B) /= 0) or else
         ((Input_A * Sol_Iterative.X) + (Input_B * Sol_Iterative.Y) = Input_C);

      Recursive_Condition :=
         (Sol_Recursive.Err and then Input_C mod GCD_Recursive_C(Input_A, Input_B) /= 0) or else
